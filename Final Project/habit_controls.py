    ...
    for i, constellation in enumerate(habits.second.constellations):
        if track > len(constellation):
            ...
            if unset:
                ...
                habits.second.cur_star = track-1
                unset = False
        for star in constellation:
...
def nextDay(habits):
    ...
    if day is not habits.day:
        ...
        next_first = True
        if habits.tracking_day == 1 and not habits.first.constellations[habits.first.cur_constellation][habits.first.cur_star].complete:

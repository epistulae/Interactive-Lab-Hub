...
class ColorPalettes(Enum):
    ...
    sweetpea = [(116, 48, 35), (220, 48, 128), (246, 118, 182), (142, 182, 93), (77, 108, 0)]
    cool_autumn = [(191, 62, 39), (239, 124, 79), (203, 149, 115), (150, 214, 227), (57, 147, 153)]
    citrus = [(196, 60, 22), (253, 154, 126), (255, 203, 143), (252, 181, 0), (220, 107, 2)]
    tropical = [(140, 161, 24), (251, 195, 26), (250, 222, 152), (234, 231, 224), (77, 150, 153)]
    ocean = [(218, 228, 229), (156, 236, 240), (105, 195, 255), (105, 130, 255), (43, 74, 74)]
    orange_juice = [(255, 106, 0), (255, 202, 42), (102, 176, 22), (155, 216, 218), (188, 206, 209)]
    plum = [(153, 7, 39), (199, 0, 96), (146, 51, 103), (201, 151, 192), (214, 204, 213)]
...
def variedFade(strip, leds, habits):
    ...
    # Constellation colors, picked from the palette
    colors = [random.choice(palette) for _ in range(len(all_constellations))]
    
    while (leds.mode is 2) and (not leds.intercept):
        for k in range(256):
            if (leds.mode is 2) and (not leds.intercept):
                for i, constellation in enumerate(all_constellations):
                    led_color = Color(int((k/256)*colors[i][0]), int((k/256)*colors[i][1]), int((k/256)*colors[i][2]))
                    for star in constellation:
                        strip.setPixelColor(star.index, led_color)
                        # Connectors
                        for prior in star.prior_stars:
